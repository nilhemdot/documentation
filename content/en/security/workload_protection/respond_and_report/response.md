...
<div class="alert alert-warning">Misconfigured enforcement rules can disrupt workloads. Test response rules in non-production environments, keep expressions tightly scoped (for example to a specific process ID or cgroup), and use the safety mechanisms described in the <a href="/security/workload_protection/workload_protection_agent_config">Workload Protection Agent configuration</a> documentation unless Support advises otherwise.</div>
